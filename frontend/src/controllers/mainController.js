...
export function initApp() {
  ...
  const elements = {
    ...
    filterWorkflow: document.getElementById('filter-workflow'),
    ...
    btnAddUserBranch: document.getElementById('btn-add-user-branch'),
  ...
  function setupAuthenticatedState() {
    ...
    // Branch filter access permissions
    if (state.currentUser.role === 'ADMIN' || state.currentUser.role === 'MANAGEMENT') {
      elements.filterBranch.disabled = false;
      elements.btnAddBranch.style.display = 'inline-block';
    } else {
      elements.filterBranch.disabled = true;
      elements.btnAddBranch.style.display = 'none';
    }

    // Bind Add Branch Event
    elements.btnAddBranch.onclick = async () => {
      const newBranch = await showCustomDialog({
        title: 'Add New Branch',
        message: 'Please enter the name of the new branch office:',
        isPrompt: true,
      ...
      if (newBranch) {
        ...
        let customBranches = JSON.parse(localStorage.getItem('kvr_custom_branches')) || [];
        if (!customBranches.includes(clean) && clean !== 'Perinthalmanna') {
          customBranches.push(clean);
          localStorage.setItem('kvr_custom_branches', JSON.stringify(customBranches));
          showToast('Success', `Branch "${clean}" added successfully.`, 'success');
          
          initBranches();
          
          // Switch to the newly created branch
          elements.filterBranch.value = clean;
          state.filters.branch = clean;
          renderVehicles();
        } else {
          showToast('Info', 'Branch already exists.', 'info');
        }
      }
    };
  ...
  async function initDashboard() {
    try {
      ...
      // Initialize branch list and dropdowns
      initBranches();
  ...
  function bindEvents() {
    ...
    const filterInputs = [
      ...
      elements.filterWorkflow, elements.filterPending,
    ...
    elements.btnClearFilters.onclick = () => {
      filterInputs.forEach(input => {
        if (input && input !== elements.filterBranch) input.value = '';
      });
      // Reset state filters (except branch)
    ...
    if (elements.btnAddUserBranch) {
      elements.btnAddUserBranch.onclick = async () => {
        const newBranch = await showCustomDialog({
          title: 'Add New Branch',
          message: 'Please enter the name of the new branch office:',
          isPrompt: true,
        ...
        if (newBranch) {
          ...
          if (!customBranches.includes(clean) && clean !== 'Perinthalmanna') {
            ...
            // Auto select the new branch in the user form
            const inpUserBranch = document.getElementById('inp-user-branch');
            if (inpUserBranch) inpUserBranch.value = clean;
          } else {
            showToast('Info', 'Branch already exists.', 'info');
          }
        }
    ...
    // User Management Admin Filters
    const filterUserRole = document.getElementById('filter-user-role');
    const filterUserBranch = document.getElementById('filter-user-branch');
    if (filterUserRole) filterUserRole.onchange = renderUsersList;
    if (filterUserBranch) filterUserBranch.onchange = renderUsersList;
  }

  // Dynamic Branch management helper
  function initBranches() {
    ...
    if (state.currentUser && state.currentUser.branch) {
      branchesSet.add(state.currentUser.branch);
    }
    state.vehicles.forEach(v => {
      if (v.branch) branchesSet.add(v.branch);
    });
    
    state.branches = Array.from(branchesSet).sort();
    
    // Populate branch filter dropdown
    const originalFilterVal = elements.filterBranch.value || (state.currentUser ? state.currentUser.branch : '') || 'Perinthalmanna';
    elements.filterBranch.innerHTML = '';
    state.branches.forEach(b => {
      elements.filterBranch.innerHTML += `<option value="${b}">${b}</option>`;
    });
    elements.filterBranch.value = originalFilterVal;
    state.filters.branch = originalFilterVal;

    // Populate vehicle drawer branch dropdown
    const inpSalesBranch = document.getElementById('inp-sales-branch');
    if (inpSalesBranch) {
      const originalInpVal = inpSalesBranch.value;
      inpSalesBranch.innerHTML = '';
      state.branches.forEach(b => {
        inpSalesBranch.innerHTML += `<option value="${b}">${b}</option>`;
      });
      if (originalInpVal && state.branches.includes(originalInpVal)) {
        inpSalesBranch.value = originalInpVal;
      }
    }

    // Populate user registration branch dropdown
    const inpUserBranch = document.getElementById('inp-user-branch');
    if (inpUserBranch) {
      const originalUserVal = inpUserBranch.value;
      inpUserBranch.innerHTML = '';
      state.branches.forEach(b => {
        inpUserBranch.innerHTML += `<option value="${b}">${b}</option>`;
      });
      if (originalUserVal && state.branches.includes(originalUserVal)) {
        inpUserBranch.value = originalUserVal;
      }
    }

    // Populate user list branch filter dropdown
    const filterUserBranch = document.getElementById('filter-user-branch');
    if (filterUserBranch) {
      const originalFilterVal = filterUserBranch.value;
      filterUserBranch.innerHTML = '<option value="">All Branches</option>';
      state.branches.forEach(b => {
        filterUserBranch.innerHTML += `<option value="${b}">${b}</option>`;
      });
      if (originalFilterVal && (state.branches.includes(originalFilterVal) || originalFilterVal === '')) {
        filterUserBranch.value = originalFilterVal;
      }
    }
  }

  // Reusable custom dialog helper for prompt / confirm replacements
  function showCustomDialog({ title, message, isPrompt = false, placeholder = '', defaultValue = '' }) {
    return new Promise((resolve) => {
      ...
      const content = overlay.querySelector('.modal-drawer');
      ...
      // Also support Enter key press in prompt
      inputEl.onkeydown = (e) => {
        if (e.key === 'Enter') {
          btnConfirm.click();
        }
  ...
  function renderKPIs() {
    const activeBranch = state.filters.branch;
  ...
  function handleFilterChange() {
    ...
    state.filters.workflow = elements.filterWorkflow.value;
    state.filters.pending = elements.filterPending.value;
  ...
  function getFilteredVehicles() {
    ...
    // 2a. Branch filter
    const vBranch = v.branch || 'Perinthalmanna';
    if (state.filters.branch && vBranch !== state.filters.branch) return false;

    // 2. Dropdown Select filters
    if (state.filters.status && v.vehicleStatus !== state.filters.status) return false;
    if (state.filters.workflow && v.vehicleStatus !== state.filters.workflow) return false;
    
    // 2b. Pending filter
    if (state.filters.pending) {
      // If the overall vehicle is already delivered or cancelled, it is not pending
      if (v.vehicleStatus === 'Delivered' || v.vehicleStatus === 'Cancelled') {
        return false;
      }
      if (state.filters.pending === 'any') {
        const statuses = DEPARTMENT_KEYS.map(key => v[SECTIONS[key].statusField] || STATUS_VALUES.NOT_ATTENDED);
        const isAllApproved = statuses.every(s => s === STATUS_VALUES.APPROVED);
        if (isAllApproved) {
          return false;
        }
      } else {
        const statusField = SECTIONS[state.filters.pending].statusField;
        const deptStatus = v[statusField] || STATUS_VALUES.NOT_ATTENDED;
        if (deptStatus !== STATUS_VALUES.PENDING && deptStatus !== STATUS_VALUES.NOT_ATTENDED) {
          return false;
        }
      }
    }
    ...
    filtered.forEach(vehicle => {
      ...
      <div class="card-details-grid">
        ...
        <div class="card-detail-item">
          ...
          <span class="card-detail-value">${vehicle.branch || 'Perinthalmanna'}</span>
        </div>
        <div class="card-detail-item">
  ...
  async function renderAuditLogs(filterKeyword = '') {
    try {
      ...
      const filtered = logs.filter(log => {
        // Filter by branch first so branches data do not collapse each other
        const matchingVehicle = state.vehicles.find(v => v.chassisNumber === log.chassisNumber);
        const logBranch = matchingVehicle ? (matchingVehicle.branch || 'Perinthalmanna') : 'Perinthalmanna';
        if (state.filters.branch && logBranch !== state.filters.branch) return false;
        ...
        sec.fields.forEach(field => {
          ...
          if (inp) {
            if (field.default !== undefined) {
              ...
              inp.value = state.currentUser.branch || 'Perinthalmanna';
            } else {
              inp.value = '';
            }
          }
  ...
  async function handleDeleteVehicle() {
    ...
    const confirmed = await showCustomDialog({
      ...
      message: `Are you sure you want to delete the booking for ${custName} (${chassis})?`
    });
  ...
  async function handleFormSubmit(e) {
    ...
    // Read branch value from form input or default to user's branch
    const branchInp = document.getElementById('inp-sales-branch');
    submissionData.branch = branchInp ? branchInp.value : (state.currentUser.branch || 'Perinthalmanna');
  ...
  async function renderUsersList() {
    try {
      ...
      // Dynamically add branches of existing users to branches list
      let branchesChanged = false;
      users.forEach(user => {
        if (user.branch && !state.branches.includes(user.branch)) {
          state.branches.push(user.branch);
          branchesChanged = true;
        }
      });
      if (branchesChanged) {
        state.branches.sort();
        // Update user form select options
        const inpUserBranch = document.getElementById('inp-user-branch');
        if (inpUserBranch) {
          const originalUserVal = inpUserBranch.value;
          inpUserBranch.innerHTML = '';
        ...
        const originalFilterVal = elements.filterBranch.value;
        elements.filterBranch.innerHTML = '';
        state.branches.forEach(b => {
          elements.filterBranch.innerHTML += `<option value="${b}">${b}</option>`;
        });
        ...
        // Update user list branch filter dropdown
        const filterUserBranch = document.getElementById('filter-user-branch');
        if (filterUserBranch) {
          const originalFilterUserVal = filterUserBranch.value;
          filterUserBranch.innerHTML = '<option value="">All Branches</option>';
          state.branches.forEach(b => {
            filterUserBranch.innerHTML += `<option value="${b}">${b}</option>`;
          });
          filterUserBranch.value = originalFilterUserVal;
        }
      }

      // Filter users locally based on filter elements
      const filterRoleEl = document.getElementById('filter-user-role');
      const filterBranchEl = document.getElementById('filter-user-branch');
      const filterRole = filterRoleEl ? filterRoleEl.value : '';
      const filterBranch = filterBranchEl ? filterBranchEl.value : '';

      const filteredUsers = users.filter(user => {
        const matchesRole = !filterRole || user.role === filterRole;
        const matchesBranch = !filterBranch || (user.branch || 'Perinthalmanna') === filterBranch;
      ...
      filteredUsers.forEach(user => {
        ...
        <td style="font-weight:500;">${user.branch || 'Perinthalmanna'}</td>
  ...
  function loadUserToEditForm(user) {
    ...
    const inpUserBranch = document.getElementById('inp-user-branch');
    if (inpUserBranch) {
      inpUserBranch.value = user.branch || 'Perinthalmanna';
    }
  ...
  function resetUserAdminForm() {
    ...
    const inpUserBranch = document.getElementById('inp-user-branch');
    if (inpUserBranch) {
      inpUserBranch.value = state.currentUser ? (state.currentUser.branch || 'Perinthalmanna') : 'Perinthalmanna';
    }
  ...
  async function handleUserFormSubmit(e) {
    ...
    const inpUserBranch = document.getElementById('inp-user-branch');
    const branch = inpUserBranch ? inpUserBranch.value : 'Perinthalmanna';

    const payload = { username, password, name, role, branch };
  ...
  async function handleDeleteUserClick(username) {
    const confirmed = await showCustomDialog({
      ...
      message: `Are you sure you want to delete the login account for "${username}"?`
    });
